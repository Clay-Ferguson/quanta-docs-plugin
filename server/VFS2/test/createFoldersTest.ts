...
export async function createFoldersTest(owner_id: number): Promise<void> {
    try {
        ...
        const folder1Exists = await vfs2.exists('/folder1');
        if (!folder1Exists) {
            throw new Error('Test 1 failed! folder1 should exist after createFolder');
        }
        
        // Verify it's a directory
        const folder1Stat = await vfs2.stat('/folder1');
        ...
        // Verify folder exists
        const folder2Exists = await vfs2.exists('/folder2');
        if (!folder2Exists) {
            throw new Error('Test 2 failed! folder2 should exist after createFolder');
        }
        
        // Verify it's a directory
        const folder2Stat = await vfs2.stat('/folder2');
        ...
        // Verify folder exists
        const folder3Exists = await vfs2.exists('/folder3');
        if (!folder3Exists) {
            throw new Error('Test 3 failed! folder3 should exist after createFolder');
        }
        
        // Verify it's a directory
        const folder3Stat = await vfs2.stat('/folder3');
