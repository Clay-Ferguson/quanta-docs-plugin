...
export async function sameFolderReorderTest(owner_id: number): Promise<void> {
    try {
        ...
        await vfs2.mkdirEx(owner_id, 'test-folder', { recursive: true }, false, 0);
        await vfs2.writeFileEx(owner_id, 'test-file.md', 'File content', 'utf8', false, 1);
        ...
        await vfs2.setOrdinal(file.uuid, 0);
        console.log('  Set file to ordinal 0');
        await vfs2.setOrdinal(folder.uuid, 1);
        ...
        await vfs2.setOrdinal(file.uuid, tempOrdinal);
        await vfs2.setOrdinal(folder.uuid, tempOrdinal + 1);
        
        // Set final ordinals
        await vfs2.setOrdinal(folder.uuid, 0);
        await vfs2.setOrdinal(file.uuid, 1);
        ...
        await vfs2.writeFileEx(owner_id, 'file2.md', 'Second file', 'utf8', false, 2);
        ...
        tempOrdinal = -2147483648;
        await vfs2.setOrdinal(folder6.uuid, tempOrdinal++);
        await vfs2.setOrdinal(file6.uuid, tempOrdinal++);
        await vfs2.setOrdinal(file2_6.uuid, tempOrdinal++);

        // Phase 2: Final ordinals
        await vfs2.setOrdinal(file2_6.uuid, 0);
        await vfs2.setOrdinal(file6.uuid, 1);
        await vfs2.setOrdinal(folder6.uuid, 2);
